<template>
    <div>
        ...
        <el-card class="box-card">
            ...
            :page-sizes="[1, 3, 6, 9]" :page-size="queryInfo.pagesize"
    ...
    methods: {
        ...
        addUser () {
            this.$refs.addFormRef.validate(async valid => {
                if (!valid) return  // valid返回的是布尔值,详见element-ui
                // 预校验通过之后，可以发起添加用户的网络请求
        ...
        const {data: res} = await this.$http.delete('users/' + id, this.editForm.id)
        if (res.meta.status !== 200) {
